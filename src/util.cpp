...
const char * const BITCOIN_CONF_FILENAME = "omegacoin.conf";
const char * const BITCOIN_PID_FILENAME = "omegacoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "omegacoin" is a composite category enabling all OmegaCoin-related debug output
            if(ptrCategory->count(string("omegacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OmegaCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OmegaCoinCore
    // Mac: ~/Library/Application Support/OmegaCoinCore
    // Unix: ~/.omegacoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OmegaCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OmegaCoinCore";
#else
    // Unix
    return pathRet / ".omegacoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty omegacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override omegacoin.conf
