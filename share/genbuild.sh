...
if [ "${BITCOIN_GENBUILD_NO_GIT}" != "1" ] && [ -e "$(command -v git)" ] && [ "$(git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]; then
    ...
    # full describe for About dialog, e.g. "v0.20.0-8-ga6bdbcf0e597"
    GIT_DESCRIBE=$(git describe --tags --long --abbrev=12 2>/dev/null)
    git diff-index --quiet HEAD -- || GIT_DESCRIBE="$GIT_DESCRIBE-dirty"
fi
