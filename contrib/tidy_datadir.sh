...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Super7Coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Super7Coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Super7Coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Super7Coin 0.7 datadir."
    ;;
  3)
    echo "Detected Super7Coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Super7Coin 0.8 datadir."
    ;;
esac
