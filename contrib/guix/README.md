...
1. Since only Windows build outputs require codesigning in this fork, the `HOSTS`
   environment variable defaults to `x86_64-w64-mingw32`.
...
```
env DETACHED_SIGS_REPO=<path/to/omega-detached-sigs> ./contrib/guix/guix-codesign
```
...
more efficient to keep the depends tree's download cache and build cache outside
of the worktrees to avoid duplicate downloads and unnecessary builds. To help
with this situation, the `guix-build` script honours the `SOURCES_PATH` and
`BASE_CACHE` environment variables and will pass them on to the depends tree so
...
* _**HOSTS**_
  ...
  _(defaults to "x86\_64-linux-gnu aarch64-linux-gnu x86\_64-w64-mingw32")_
