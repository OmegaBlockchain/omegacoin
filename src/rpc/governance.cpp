...
static void gobject_vote_conf_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"gobject vote-conf",
        "Vote on a governance object by masternode configured in omega.conf\n",
        {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("omega.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by masternode configured in omega.conf\n"
#ifdef ENABLE_WALLET
