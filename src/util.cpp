...
const char * const BITCOIN_CONF_FILENAME = "super7coin.conf";
const char * const BITCOIN_PID_FILENAME = "super7coind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "super7coin" is a composite category enabling all Super7Coin-related debug output
            if(ptrCategory->count(string("super7coin"))) {
...
#else
    const char* pszModule = "super7coin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Super7CoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Super7CoinCore
    // Mac: ~/Library/Application Support/Super7CoinCore
    // Unix: ~/.super7coincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Super7CoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Super7CoinCore";
#else
    // Unix
    return pathRet / ".super7coincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty super7coin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override super7coin.conf
