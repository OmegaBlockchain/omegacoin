...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. OmegaCoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(omegacoin);
    Q_INIT_RESOURCE(omegacoin_locale);
    ...
    /// 6. Determine availability of data directory and parse omegacoin.conf
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("OmegaCoin Core"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("OmegaCoin Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
