...
void CMasternode::Check(bool fForce)
{
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        LogPrint("masternode", "CMasternode::Check -- outpoint=%s, GetAdjustedTime()=%d\n",
                vin.prevout.ToStringShort(), GetAdjustedTime());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 OMEGA, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 OMEGA tx got nMasternodeMinimumConfirmations
