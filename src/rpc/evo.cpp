    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        ...
        {"platformP2PPort",
            {"platformP2PPort", RPCArg::Type::NUM, RPCArg::Optional::NO,
                "TCP port of Omega Platform peer-to-peer communication between nodes (network byte order)."}
        },
        {"platformHTTPPort",
...
static void protx_register_fund_help(const JSONRPCRequest& request, bool legacy)
{
    ...
    RPCHelpMan{rpc_full_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Omega\n"
...
static void protx_register_fund_hpmn_help(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 4000 Omega\n"
        "to the address specified by collateralAddress and will then function as the collateral of your\n"
        "HPMN.\n"
        "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
        "is fully deployed.\n" +
                                              ...
                                              const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Omega address: ") + request.params[paramIdx + 6].get_str());
    }
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 1].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Omega address: ") + request.params[paramIdx + 1].get_str());
    } else {
...
static UniValue protx_update_registrar_wrapper(const JSONRPCRequest& request, const bool specific_legacy_bls_scheme)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Omega address: ") + request.params[4].get_str());
    }
...
static UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Omega address: ") + request.params[3].get_str());
        FundSpecialTx(wallet.get(), tx, ptx, feeSourceDest);
