 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called OmegaCoin (https://www.omegacoin.org/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "omegacoin:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in omegacoind anymore. Use the omegacoin-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "OmegaCoin Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect omegacoind signal handlers
    noui_connect();
